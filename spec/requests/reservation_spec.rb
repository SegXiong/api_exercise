...
RSpec.describe "API_V1::Reservations", :type => :request do
  ...
  example "PATCH /api/v1/reservations/{booking_code}" do
    patch "/api/v1/reservations/#{@reservation.booking_code}", :params => { :customer_name => "bar", :customer_phone => "87654321"}
    expect(response).to have_http_status(200)
  ...
  describe "POST /api/v1/reservations" do
    example "success without auth_token" do
      post "/api/v1/reservations", :params => { :train_number => @train1.number, :seat_number => "1B", :customer_name => "zoo", :customer_phone => "55555555"}
      expect(response).to have_http_status(200)
      created_reservation = Reservation.last
      expect(response.body).to eq( { :booking_code => created_reservation.booking_code, :reservation_url => api_v1_reservation_url(created_reservation.booking_code) }.to_json )
    ...
    example "success with auth_token" do
      post "/api/v1/reservations", :params => { :auth_token => @user.authentication_token, :train_number => @train1.number, :seat_number => "1B", :customer_name => "zoo", :customer_phone => "55555555"}
      expect(response).to have_http_status(200)
      created_reservation = Reservation.last
      expect(response.body).to eq( { :booking_code => created_reservation.booking_code, :reservation_url => api_v1_reservation_url(created_reservation.booking_code) }.to_json )
      expect(created_reservation.customer_name).to eq("zoo")
  ...
  describe "GET /api/v1/reservations" do
    ...
    example "success" do
      @reservation1 = Reservation.create!( :user_id => @user.id, :train_id => @train1.id, :seat_number => "1B", :customer_name => "foo", :customer_phone => "12345678")
      @reservation2 = Reservation.create!( :user_id => @user.id, :train_id => @train1.id, :seat_number => "1C", :customer_name => "foo", :customer_phone => "12345678")
